...
**Monorepo layout:** Next.js lives in `frontend/`. Set **Root Directory** = `frontend` in Vercel → Project Settings → General (or run `./scripts/vercel-set-frontend-root.sh` with `VERCEL_TOKEN`). [`frontend/vercel.json`](../frontend/vercel.json) holds install/build. Before pushing UI changes, run `./scripts/vercel-build-check.sh` or `./scripts/ci-local.sh`.
